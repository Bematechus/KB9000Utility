...
namespace KB9Utility
{
  ...
  class KB9Logger
  {
      ...
      public const string LOG_LF = "\r\n";
      public const string LOG_TAB = "\t\t";
    ...
    #region Construction/destruction
      ...
      static private KB9Logger _Logger = null;
      static public KB9Logger Instance()
      {
          if (_Logger == null)
              _Logger = new KB9Logger( Util.GetAppPath() + "\\KB9Log.txt", true, (uint)Level.All);
          return _Logger;
      }
      static public void EnableLog(bool bEnable)
      {
          if (bEnable)
              KB9Logger.Instance().Start();
          else
              KB9Logger.Instance().Stop();
      }
      static public void Log(string strLog)
      {
          if (KB9Logger.Instance().LoggerState == State.Running)
              KB9Logger.Instance().Info(strLog);
      }
      static public void Err(string strLog)
      {
          if (KB9Logger.Instance().LoggerState == State.Running)
              KB9Logger.Instance().Error(strLog);
      }
    #endregion
    ...
    #region Operations
      ...
      public bool Start()
      {
        lock (this) {
          ...
          // Return successfully
          _state = KB9Logger.State.Running;
          return true;
        }
      }
      ...
      public bool Stop()
      {
        lock (this) {
          ...
          _state = KB9Logger.State.Stopped;
          return true;
        }
      }
